...
~~3:将iga注册进maker，可获取到 clientId 和 clientSecret~~
...
| 忽略表达式   | =if($ENTITY.source!='xxx'){xxx}else{$SOURCE} 或  $ENTITY.phone | 字段映射 对数据库某列值做忽略上游覆盖处理,可指定所有为数据库原有列值或根据条件进行选择性忽略                     |
